...
// ✅ UPDATED: Now accepts dynamic font size and styling parameters
export const drawTextSection = (
  ...
  // ✅ Use dynamic font sizes instead of hardcoded ones
  const brandFontSize = isPreview ? (metaFontSize * 0.85) : (metaFontSize * 3.375 * 0.85);
  const dynamicTitleFontSize = isPreview ? titleFontSize : (titleFontSize * 3.375);
  const footerFontSize = isPreview ? (metaFontSize * 0.7) : (metaFontSize * 3.375 * 0.7);
  const dynamicLineHeight = isPreview ? (titleFontSize * lineHeight) : (titleFontSize * 3.375 * lineHeight);
  ...
  ctx.textAlign = textAlignment; // Use dynamic alignment
